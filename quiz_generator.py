...
ALLOWED_EXTENSIONS = {
    ...
    'docx', 'xlsx', 'xls', 'ppt', 'pptx', 'odt', 'rtf',
    'json', 'xml', 'md', 'log',
    'py', 'js', 'html', 'css', 'java', 'c', 'cpp', 'h', 'sql', 'sh', 'bat',
    'mp3', 'mp4', 'wav', 'mov', 'avi',
    'zip', 'rar', '7z', 'tar', 'gz',
}
ALLOWED_QUESTION_TYPES = {'MCQ', 'Very Short Answer', 'Short Answer', 'Long Answer', 'Essay'}
...
def extract_text_from_file(file_content: bytes, filename: str) -> str:
    if not allowed_file(filename):
        raise ValueError('Unsupported file type. Please upload PDF, PNG, JPG, JPEG, TXT, or CSV.')
...
def _pick_keyword(sentence: str) -> str:
    ...
    return random.choice(candidates) if candidates else words[len(words) // 2]


def _pick_wrong_options(sentences, correct_word):
    pool = []
    for s in sentences:
        for w in s.split():
...
def _pick_sentences(sentences, count=1):
    return random.sample(sentences, min(count, len(sentences)))


def _make_mcq(sentence, difficulty, answer_length, points):
    ...
    if difficulty == 'Basic':
        ...
        question_text = f'[{points} mark] Which word best fits this context: "{sentence[:80]}..."?'
    else:
    ...
    for idx in range(num_questions):
        ...
        if question_type == 'MCQ':
            ...
            if answer_length == '1-line':
                ...
                q['answer'] = f'{word} – it is the key term identified in the given context.'
            elif answer_length == 'Detailed':
                q['answer'] = f'{word} is the correct answer. It matches the context of: "{sentence[:60]}..." It carries significant meaning here.'
            else:
                q['answer'] = (
            ...
            if difficulty == 'Basic':
                ...
                q['question'] = f'[{points} mark] State the meaning of "{word}" from the text.'
                q['answer'] = f'{word} refers to the key concept in the passage.'
            else:
                q['question'] = f'[{points} mark] Why is "{word}" significant in the passage?'
                q['answer'] = f'{word} is significant because it connects multiple ideas in the passage.'
            q['answer'] = q['answer'] if answer_length == '1-line' else (q['answer'] + ' ' + sentence[:60] + '...')
            ...
            if difficulty == 'Basic':
                ...
                q['question'] = f'[{points} mark] Describe briefly how "{word}" affects the passage.'
                q['answer'] = f'{word} has a direct impact on the passage meaning. It helps readers understand the central theme better.'
            else:
                q['question'] = f'[{points} mark] Pro-level: Explain the role of "{word}" in this context with one example.'
                q['answer'] = f'{word} plays a key role by reinforcing the theme. For example, in the sentence "{sentence[:60]}...", it elevates the overall message.'
            q['answer'] = q['answer'] if answer_length != 'Detailed' else (q['answer'] + ' ' + sentence[:60] + '...')

        elif question_type == 'Long Answer':
            if difficulty == 'Basic':
                q['question'] = f'[{points} mark] Write a short note on: {sentence[:80]}...'
                q['answer'] = f'{word} is important. It helps explain the main point and gives context to the passage.'
            elif difficulty == 'Intermediate':
                q['question'] = f'[{points} mark] Describe the importance of "{word}" in: {sentence[:80]}...'
                q['answer'] = f'{word} contributes greatly. It supports the argument and adds depth to the understanding of the topic. Without it, the passage would be incomplete.'
            else:
                q['question'] = f'[{points} mark] Analyze the importance of "{word}" in: {sentence[:80]}...'
                q['answer'] = (
            ...
            if answer_length in ('1-line', '2-line'):
                q['answer'] = '. '.join(q['answer'].split('. ')[:2]) + '.'
            ...
            q['question'] = f'[{points} mark] Essay: Discuss in detail the importance of "{word}" with reference to: {sentence[:80]}...'
            q['answer'] = (
            ...
            if answer_length == '1-line':
                ...
                q['answer'] = f'{word} is important in context. It contributes to the central theme.'
    ...
    for attempt in range(3):
        try:
            ...
            for idx, item in enumerate(data['quiz'][:num_questions]):
                ...
                q_type_raw = (item.get('type') or '').strip().lower().replace(' ', '_').replace('-', '_')
                q_type_map = {
                    'mcq': 'MCQ',
...
def generate_quiz_locally(text_content: str, num_questions: int = 10, question_type: str = 'MCQ'):
